...
- 同一用户对同一张图只保留一票；重复投票的取值策略可配：`last_wins`（默认）、`first_wins`、`max_score`、`min_score`。
- 同一个角色的多张图，除按图排名外还会给一份按角色合并的统计（角色名默认取文件名里第一个短横线之前的部分，可用项目目录的 `project.json` 覆盖）。
...
其余包括最后一张的额外等待、权限控制（仅管理员可开始、群白名单）、重复投票策略、报告压缩参数、AI 开关与 Top/Bottom 数量、发送重试次数、连续发送失败自动暂停阈值、结束时是否在群里提醒、是否自动生成报告、单文件报告是否发到群里、是否按固定周期发送等。WebUI 保存后配置会在下一条指令时自动生效，想立刻确认就打 `/vote reloadconfig` 看实际生效值。

关于发送间隔：默认含义是「上一张发送完成后等待 N 秒」，实际出图间隔 = 图片上传耗时 + N（大图上传慢时会明显拉长）；把 `interval_includes_send_time` 打开会改成周期制，等待时间 = max(0, N − 本张发送耗时)。
