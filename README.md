...
其余包括最后一张的额外等待、权限控制（仅管理员可开始、群白名单）、报告压缩参数、AI 开关与 Top/Bottom 数量、发送重试次数、连续发送失败自动暂停阈值、结束时是否在群里提醒、是否自动生成报告、单文件报告是否发到群里、是否按固定周期发送等。WebUI 保存后配置会在下一条指令时自动生效，想立刻确认就打 `/vote reloadconfig` 看实际生效值。
