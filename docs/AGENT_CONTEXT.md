...
> 整理日期：2026-09-16。代码基线：`a1e3dc9`，插件版本 `0.13.5`。
...
| 测试 | Python unittest；另有隔离 HTTP、大图及报告预览工具 |
...
旧配置 `interval_includes_send_time` 仍兼容读取，但已废弃、不生效。不要恢复成“发送耗时抵扣人物投票间隔”的旧逻辑。
...
正式管理页分为 **运行 / 项目 / 历史与报告 / 设置**：选择群、项目预检与启动，暂停和继续，目录登记与预览，报告预览下载重生成，清理或彻底删除，分组配置保存。
...
0.13.0 完成人物口径迁移；0.13.1–0.13.2 增加合并发送、拆条上限和超时；0.13.3–0.13.4 统一中文状态与投票消息结构；0.13.5 修复合并发送的节奏警告判定和报告排名卡排版。

本次整理重新运行单元测试：共 133 项，132 项通过、1 项跳过；当前系统 Python 缺少 Pillow，图片相关用例跳过。本次未重新连接生产实例或运行隔离 HTTP 验收。

本会话上一轮对报告排版做过本地浏览器验收：目录版与单文件版，320 / 390 / 640 / 900 / 1440px，单卡和三卡不溢出、不重叠，导航与图片弹窗可用，检查了明暗主题截图。该结果来自本地示例，不代表截图中真实历史报告已被重新导出。

历史验收记录覆盖真实 QQ 发送、报告对账及部分管理 API；但各记录基于不同版本。`docs/IMPLEMENTATION_STATUS.md` 仍混有 0.12.x 与 0.13.0 状态，`docs/ARCHITECTURE.md` 的“下一步”也有已完成内容。判断当前功能时优先阅读代码和最新 CHANGELOG，不能把历史测试数、已知缺口或部署记录当成当前在线事实。

接手原则：

1. 保持人物投票口径、平台适配边界与共享应用服务，不另建重复业务入口。
2. 修报告查 `assets/report.*` 与 `report_generator.py`；修管理页查 `pages/image-vote/` 与 workspace 服务，别误改原型。
3. 已生成的报告是静态快照，更新插件源码不会自动更新历史 HTML，需要重新导出。
4. 本地测试、隔离验收、真实群验证与部署完成应分别报告。
5. 迁移、生产数据清理、部署与群消息发送需按用户授权范围执行；本文不授予这些操作权限。
6. 整理时工作区已有 `.gitignore` 修改，未归入本文任务；后续提交前重新检查工作区。
