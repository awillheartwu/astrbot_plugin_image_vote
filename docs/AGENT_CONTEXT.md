...
> 整理日期：2026-09-16（已发布基线 `0.13.12`，本轮为 0.13.13 dev 发布前修订）。代码基线见最新 `CHANGELOG.md`；本文描述的能力与边界截至该版本。
...
| `tools/sample_project.py` | 动态生成小型测试图片，替代已删除的旧原型与插画 |
...
本轮在带 Pillow 的环境运行 151 项单元测试。管理页检查覆盖明暗主题、三种宽度及预检渐进加载；具体最新结果见 IMPLEMENTATION_STATUS.md。历史真机场次验证不能替代本轮上线验收。
...
3. 已生成的报告是静态快照，更新插件源码不会自动更新历史 HTML；重新导出一次一场。不得擅自清空生产或测试历史；数据迁移和删除需明确授权。
...
本轮预检已改为元数据先返回、图片及群列表独立渐进加载；缩略图内存和磁盘缓存有上限，新配置 output_root 留空默认集中到插件数据目录 reports/。已有路径不迁移。详情见 DATA_MAINTENANCE.md。
