...
> 2026-09-16 更新：本文件保留为历史路线图。0.13.x 已完成按人物发送与计票、合并发送与拆条、报告页重做（深色面板与离线图表）、插件工作区四个页面，以及报告 / 面板的浏览器检查工具；下方第二、三节的差距表多为历史条目（N1/N3/N4/N5/N6、`export --ai`、标准差展示、模板清理等均已完成）。当前实现与验收状态以 [IMPLEMENTATION_STATUS.md](IMPLEMENTATION_STATUS.md) 为准。
>
> 历史测试数据的处置需要明确授权，不得依据本路线图自动清空；当前行为以实现与验收记录为准。
...
## 十二、已完成：消息与报告命名修正（2026-09-12）

- 投票消息不再重复短 ID：第二行改为「项目名 · 图片标题」，冗余的 `[VOTE:短ID:序号]` 机器标记不再发送（解析器保留对历史消息的兼容）。
- 登记项目与别名项目的会话改用「调用时使用的项目名」，报告目录从文件夹名（如 `人物图`）改为项目名（如 `海滨之家`）。
- 控制指令回复的末行改为 `项目：X · Session：Y · 状态：Z`，不再只给会话 ID。
- 老会话自动纠正名字：运行或导出时若 `project_path` 命中登记表，会话项目名更改为登记名并落库，报告目录随之统一；`save_session` 的 UPSERT 现在也会更新 `project_name`。

## 十三、已完成：配置热读取与报告发群（2026-09-12）

- 修掉「UI 里改了发送间隔 / 报告模式但没生效」：配置解析改为优先读 `astrbot_config_mgr` 与磁盘上的 `<data>/config/<插件名>_config.json`，其次才是可能过期的实例属性与 `context.get_config()`；文件按 (mtime, size) 缓存，保存后下一条指令即生效。
- 新增 `/vote reloadconfig`（管理员）：强制重新读取配置并打印实际生效值，用于确认 UI 保存有没有传到运行中的插件。
- 新增 `send_report_html`（默认关）：`report_mode = single_html` 时把单文件报告作为附件发到投票群；目录模式不发送文件。适配层新增 `send_file`，发送失败只记警告，不影响报告本身。
- 报告生成时打印实际使用的模式，便于排查。

## 十四、已完成：间隔语义与单文件产物清理（2026-09-13）

- 历史功能（现已删除）：曾新增固定周期配置（默认 false）：开启后按固定周期发送，等待时间 = max(0, 间隔 − 本张发送耗时)，解决「设 5 秒、实测 20 秒」的预期差（差值是图片上传耗时）。
...
## 十六、2026-09-17 交接：已完成与后续计划

本轮（0.13.6–0.13.14）已完成：报告页重做（默认深色作品面板、离线 SVG 图表、图册/列表、参与者评分表、封面策略）、插件工作区重做（项目封面卡、图标、明暗主题、预检分批加载与缩略图两级缓存）、维护与缓存规范化（归属标记临时目录、启动与每小时清理、缓存集中到插件数据目录）、发布流程（LICENSE、CONTRIBUTING、RELEASE.md、check_release.py、deploy_nas.sh、双远端推送与 v0.13.14 标签）、文档重写（README 面向用户，USER_GUIDE / DATA_MAINTENANCE 承载细节）。

真机已验证：63 张 / 31 人物 / 15 参与者 / 244 票的完整场次（榆木大学 7473A4F0），发送 0 失败、节奏符合人物窗口、18 张引用票正常、报告与数据库一致。

### 后续待办（按优先级，下次开发从这里继续）

**P0｜发布收尾**

1. 插件市场提交（AstrBot Cloud + GitHub 仓库地址），提交后验证市场下载链路与首次安装体验。
2. 已完成：`v0.13.13` 之后的本地修复（面板筛选加固、报告长昵称单行省略、报告工具栏窄屏、面板空态引导、manifest 命中时展示标题改用人物名）已合并进 main 并发布为 `v0.13.14`。

**P1｜数据语义（建议先与用户确认）**

3. 「0 分」的含义：真机场次里 29 张 0 分票直接计入均分，把长尾压到 1.1–1.3 分。可选方案：报告标注 0 分票数、增加「0 分不计入均分」开关，或至少在口径说明写明「0 分按 0 分计入」。
4. 参与者覆盖差异：DanDy 只评 5 个人物、妖桃评 30 个，均分里同权。报告已有「仅 N 票」小样本标记，可在概览补充「N 位参与者评分覆盖不足 1/3」的提示。

**P2｜体验一致性**

5. 报告浅色主题专门设计：目前是深色 token 反相，头部封面在浅色下仍是深色剧照，图表配色未单独取值。
6. 报告窄屏（320/390）在新版下的复验；工具栏已改为胶囊字段 + 下拉箭头（本次），但图册/列表切换在小屏的可发现性仍可再评估。
7. 面板断连时保留页面框架（当前整块替换为「连接失败 + 重试」，会丢掉顶部与标签）。

**P3｜运维与工程**

8. 历史场次的数据库增长：每场 N 行 candidates，目前无保留期，可增加「按保留期清理历史场次」选项，与报告保留期解耦。
9. `auto_cleanup_reports` 默认关闭，长期运行可能堆积报告；可考虑首次运行或首次生成报告时在面板给一次提示。
10. 发布流程再脚本化一步：`tools/deploy_nas.sh` 已覆盖 NAS 部署；还可加 dev→main 合并、打标签、双远端推送的一键脚本。
11. CI 仍未接入（用户判定为可选项）。

**未验收边界（沿用）**

12. 单文件报告 `file://` 双击、真实头像下载、跨进程重启 E2E、500 MB 真实照片、多群并行、AstrBot 4.4.x。

**明确不做的**

13. 旧报告/旧配置/旧数据的兼容与批量重导：正式上线前会清空测试数据，一次性重导即可；报告重新导出保持一次一场。
