...
本轮（0.13.6–0.13.13 + 未发布修复）已完成：报告页重做（默认深色作品面板、离线 SVG 图表、图册/列表、参与者评分表、封面策略）、插件工作区重做（项目封面卡、图标、明暗主题、预检分批加载与缩略图两级缓存）、维护与缓存规范化（归属标记临时目录、启动与每小时清理、缓存集中到插件数据目录）、发布流程（LICENSE、CONTRIBUTING、RELEASE.md、check_release.py、deploy_nas.sh、双远端推送与 v0.13.13 标签）、文档重写（README 面向用户，USER_GUIDE / DATA_MAINTENANCE 承载细节）。
...
2. 已发布版本 `v0.13.13` 之后的本地修复（面板筛选加固、报告长昵称单行省略、报告工具栏窄屏、面板空态引导）需要合并进 main 并打新标签；dev 当前领先 main。
