...
以下场景在 `docs/REQUIREMENTS.md` §11.7 有完整表格，这里列出需要拍板的几条：

1. 机器人自己发的数字消息是否跳过（比对 `self_id`）。
2. 发送失败的图片是否还接受投票；是否改为「保留上一张为当前窗口」。
3. 连续发送失败达到阈值时是否自动暂停并通知管理员。
4. 短 ID 是否从 4 位提高到 6-8 位，或延长内部标记长度。
