...
(function () {
  ...
  function runPage() {
    return `<div class="admin-stats"><span><b>${active.length}</b>场进行中</span><span><b>${projects.length}</b>个项目</span></div>${active.length ? active.map((r) => `<section class="run-grid"><div class="surface"><div class="label-row"><div><h2>${esc(r.project_name)}</h2><p class="subtle">群 ${esc(r.group_id)} · ${esc(r.short_id)}</p></div><span class="badge">${status[r.status] || esc(r.status)}</span></div>${r.active_candidate ? `<img class="run-image" data-thumb-session="${esc(r.id)}" data-thumb-candidate="${esc(r.active_candidate.id)}" alt="${esc(r.active_candidate.name)}">` : '<p class="muted">尚未成功发送图片</p>'}<div class="label-row"><b>${esc(r.active_candidate?.name || "等待开始")}</b><span>${r.sent_count} / ${r.candidate_count} 张已发送</span></div><div class="progress"><i style="width:${r.candidate_count ? (r.sent_count / r.candidate_count) * 100 : 0}%"></i></div><p class="subtle">${r.vote_count} 张有效票 · ${r.participant_count} 人参与${r.seconds_until_next != null ? " · 下一张约 " + Math.ceil(r.seconds_until_next) + " 秒" : ""}</p>${r.error_message ? `<p class="state-details">${esc(r.error_message)}</p>` : ""}<div class="actions">${["RUNNING", "PAUSED"].includes(r.status) ? `<button class="primary" data-control="${r.status === "PAUSED" ? "resume" : "pause"}" data-id="${esc(r.id)}">${r.status === "PAUSED" ? "继续" : "暂停"}</button><button data-control="finish" data-id="${esc(r.id)}">提前结束</button><button class="quiet danger" data-control="stop" data-id="${esc(r.id)}">取消本轮</button>` : '<span class="muted">正在收尾，请稍候</span>'}</div></div><aside class="surface"><h3>本轮参数</h3><div class="kv"><span>评分范围</span><b>${r.score_min}–${r.score_max}</b><span>发送间隔</span><b>${r.interval_seconds} 秒</b><span>最后一张等待</span><b>${r.final_grace_seconds} 秒</b><span>报告状态</span><b>${reportStatus[r.report_state]}</b></div><div class="notice">修改默认评分范围和间隔，不会改变本轮已经保存的参数。</div></aside></section>`).join("") : '<section class="surface empty"><h2>现在没有进行中的投票</h2><p>选择一个项目，预检后开始下一场。</p><button class="primary" data-action="start">选择项目</button></section>'}`;
  }
