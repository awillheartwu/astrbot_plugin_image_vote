...
async def main():
    ...
    session = Session('preview-session', 'PREVIEW', '示例讨论组', 'preview', '海滨之家 · 示例报告',
                      str(source), status=SessionStatus.COMPLETED, score_max=5, candidate_count=3,
    ...
    candidates = [Candidate('image-%s' % i, session.id, i, name+'.png', name+'.png', title,
                            None, (source/(name+'.png')).stat().st_size, send_status=SendStatus.SENT)
                  for i, (name, title) in enumerate([('alice', 'Alice · 海风与她'),
                  ('grace', 'Grace · 午后小憩'), ('iris', 'Iris · 树荫下的回眸')], 1)]
    names = ['星海与风', '柠檬汽水', '花间旅人', '晚风', 'Mirage', '海盐冰', '青禾', '山间来信']
    scores = [[5, 5, 4, 5, 4, 5, 4, 5], [4, 5, 4, 4, 5, 4, 4, 4], [5, 2, 4, 2, 5, 3, 4, 1]]
    votes = [Vote(None, session.id, c.id, 'demo-%d' % i, names[i], score, VoteSource.CURRENT_WINDOW,
                  created_at=session.started_at, updated_at=session.finished_at)
             for c, values in zip(candidates, scores) for i, score in enumerate(values)]
